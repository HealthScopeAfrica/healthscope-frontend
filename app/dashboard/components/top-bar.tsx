    ...
    <header className="border-b border-border bg-white">
      <div className="container flex items-center justify-between gap-4 py-3 xl:py-4">
        ...
        <nav className="flex items-center gap-4">
          {controls.map(({ id, icon: Icon, label, to }) =>
            to ? (
              <Link
                key={id}
                to={to}
          ...
          <Link
            to="/dashboard/profile"
            className="hidden items-center gap-2 rounded-full border border-gray-100 bg-white px-5 py-4 text-xl font-medium text-text-strong hover:border-[#C1C1C1] sm:flex"
          >
